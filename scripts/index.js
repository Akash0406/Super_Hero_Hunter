...
const access_token = '622793748868217';
const url = 'https://superheroapi.com/api.php/' + access_token + '/search/';
const favFalse = '../assets/images/white_star.png';
const favTrue = '../assets/images/blue_star.png';


const searchBar = document.getElementById('searchData');
...
searchBar.addEventListener('keyup', (e) => {
    ...
    if (searchString.length < 3) { // avoiding huge number of search results
        document.getElementById('results').innerHTML = 'Write Atleast 3 Characters';
    } else {
...
document.addEventListener('click', (event) => {
    ...
    if (event.target.id == 'details_btn') {
        ...
        window.open('../pages/details.html' + '?id=' + id, "_self");
    }
    // Favourite button
        ...
        if (favs.indexOf(id) != -1) {
            ...
            customAlert('failure', 'Removed From Favourities');
        } else {
            favs.push(id);
            localStorage.setItem('superheroFavs', JSON.stringify(favs));
            event.target.src = favTrue;
            customAlert('success', 'Added To Favourities');
        }
    }
});
...
function renderData(data) {
    ...
    if (data.response == 'error' || data.results.length === 0) {
        ...
        var result_container = document.getElementById('resultContainer');
